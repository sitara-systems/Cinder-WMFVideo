...
ciWMFVideoPlayer::ScopedVideoTextureBind::ScopedVideoTextureBind(const ciWMFVideoPlayer& video, uint8_t textureUnit)
    : mCtx(gl::context()), mTarget(video.mTex->getTarget()), mTextureUnit(textureUnit), mPlayer(video.mPlayer) {
    mPlayer->mEVRPresenter->lockSharedTexture();
    mCtx->pushTextureBinding(mTarget, video.mTex->getId(), mTextureUnit);
}

ciWMFVideoPlayer::ScopedVideoTextureBind::ScopedVideoTextureBind(const std::shared_ptr<ciWMFVideoPlayer> video,
                                                                 uint8_t textureUnit)
...
ci::ImageSourceRef ciWMFVideoPlayer::getImageSource() {
    ...
    glCopyImageSubData(mTex->getId(), mTex->getTarget(), 0, 0, 0, 0, copyTex->getId(), copyTex->getTarget(), 0, 0, 0, 0,
                       w, h, 1);
...
ciWMFVideoPlayer::ciWMFVideoPlayer() : mPlayer(NULL), mVideoFill(VideoFill::FILL), mPlayPending(false) {
    if (mInstanceCount == 0) {
...
ciWMFVideoPlayer::~ciWMFVideoPlayer() {
    ...
    g_WMFVideoPlayers.erase(std::remove_if(g_WMFVideoPlayers.begin(), g_WMFVideoPlayers.end(),
                                           [hwnd](PlayerItem& p) { return p.first == hwnd; }),
...
void ciWMFVideoPlayer::play() {
    ...
    if (mPlayer->GetState() == OPEN_PENDING) {
        mWaitForLoadedToPlay = true;
    }
...
float ciWMFVideoPlayer::getHeight() {
    return mPlayer->getHeight();
}
float ciWMFVideoPlayer::getWidth() {
...
BOOL ciWMFVideoPlayer::InitInstance() {
    ...
    // Create the application window.
    hwnd = CreateWindow(szWindowClass, L"", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, NULL,
                        NULL);
